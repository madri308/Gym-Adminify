                      ...
                      <Disclosure :colors="''" v-bind:title="activity.dayofmonth+'/'+act.schedule.month+'/'+act.schedule.year">
                        <div class="relative">
                          ...
                          <div v-if="canChangeActivity"> 
                            <button v-if="!(changing === '/'+activity.id+'/') && canSelectTeacher" @click="changing = '/'+activity.id+'/'" type="button" class="absolute top-0 right-0 -mr-1 p-1 rounded-md transition hover:bg-indigo-200 focus:outline-none focus:ring-2 focus:ring-white sm:-mr-2">
                              <i class="fas fa-pencil-alt fa-md"></i>
                            </button> 
